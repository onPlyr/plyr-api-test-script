...
// Is Inited? -- Check if the badge contract is inited
async function isInitedBadge(gameId) {
    const timestamp = Date.now().toString();
    ...
    try {
        let ret = await axios.get(
            apiEndpoint + "/api/game/badge/isInited?gameId=" + gameId,
            {
                headers: {
                    apikey: apiKey,
                    signature: hmac,
...
async function badgeList(plyrId) {
    ...
    try {
        let ret = await axios.get(
            apiEndpoint + "/api/game/badge/list?plyrId=" + plyrId,
            {
                headers: {
                    apikey: apiKey,
                    signature: hmac,
