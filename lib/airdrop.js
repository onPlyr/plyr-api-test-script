...
/*
[GET] airdropAllClaimableRewards()
 Airdrop - All Claimable Rewards with address to check
*/
async function airdropAllClaimableRewards(address) {
    const timestamp = Date.now().toString();
    let hmac = generateHmacSignature(timestamp, {}, secretKey);

    try {
        let ret = await axios.get(
            apiEndpoint + "/api/airdrop/allClaimableReward/" + address,
            {
                headers: {
                    apikey: apiKey,
                    signature: hmac,
...
*/
async function airdropUserStats(campaignId, address) {
    const timestamp = Date.now().toString();
    ...
    try {
        let ret = await axios.get(
            apiEndpoint + "/api/airdrop/campaign/" + campaignId + "/userReward/" + address,
            {
                headers: {
                    apikey: apiKey,
                    signature: hmac,
...
/*
[POST] airdropAllClaim()
params:
    address - should be a primary address only not mirror
    playedGame - true or false. True = Fully amount , False 20% penaly
*/
async function airdropAllClaim(address, playedGame) {
    const timestamp = Date.now().toString();
    let body = {
        address: address,
        playedGame: playedGame,
    ...
    try {
        let ret = await axios.post(
            apiEndpoint + "/api/airdrop/claimAllClaimableReward/",
            body,
            {
                headers: {
                    apikey: apiKey,
...
module.exports = { airdropCampaignInfo, airdropCampaignClaimableRewards, airdropCampaignClaim, airdropUserStats, airdropAllClaimableRewards, airdropAllClaim };